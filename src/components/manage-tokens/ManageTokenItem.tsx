        ...
        {token.lists?.includes("custom_ethereum") &&
          <>
            ...
            <Dialog isOpen={deleteOpened} setIsOpen={setDeleteOpened}>
              ...
              Are you sure you want delete this token? It will be removed only from your custom token list.
      ...
      <div className="px-5 pt-3 w-[530px]">
        <div className="border-b border-primary-border pb-3 flex flex-col gap-2">
          <div className="flex justify-between"><span
            className="text-secondary-text">Name</span><span>{token.name}</span></div>
          <div className="flex justify-between"><span
            className="text-secondary-text">Symbol</span><span>{token.symbol}</span></div>
          <div className="flex justify-between"><span
            className="text-secondary-text">Address</span><span>{token.address}</span></div>
          <div className="flex justify-between"><span
            className="text-secondary-text">Decimals</span><span>{token.decimals}</span></div>
        </div>
        <p className="pt-3 text-secondary-text">Found in these token-lists:</p>
        <div className="flex flex-col gap-3 py-3">
          {token.lists?.map((listId) => {
            return <div className="flex gap-3 items-center" key={listId}>
              <div className="px-2"><Image width={32} height={32} src="/token-lists/BNB.svg" alt=""/></div>
              {listId}
            </div>
          })}
        </div>
